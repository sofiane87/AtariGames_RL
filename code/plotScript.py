...
# 	#### parameters of the question
# 	questionNumber = 6
# 	iterationsToAvoid = [16]
# 	numberOfIterations = 1
# 	secondNumberOfIterations = 100
# 	learningRates = [0.015,0.01]
# 	discount = 0.99
# 	multiplePlotsName = ['Q6', 'Q4']

# 	#### paths and arrays
# 	numpyFilePaths = ['Q{}'.format(questionNumber) + os.sep + 'Numpy' + os.sep, 'Q{}'.format(4) + os.sep + 'Numpy' + os.sep ]
# 	saveFilePath  = 'report' + os.sep + 'plots' + os.sep + 'a{}'.format(questionNumber) + os.sep
# 	listOfPlots = ['bellmanLoss', 'loss',  'reward', 'train_step','test_step']
# 	iterations = [i for i in range(numberOfIterations) if i not in iterationsToAvoid]
# 	fullIterations = [i for i in range(secondNumberOfIterations) if i not in iterationsToAvoid]
...
# 	numberOfIterations = 1
# 	secondNumberOfIterations = 100
# 	learningRates = [0.00001, 0.015,0.01]
# 	discount = 0.99
# 	multiplePlotsName = ['Q7', 'Q6', 'Q4']

# 	#### paths and arrays
# 	numpyFilePaths = ['Q{}'.format(questionNumber) + os.sep + 'Numpy' + os.sep, 'Q{}'.format(6) + os.sep + 'Numpy' + os.sep , 'Q{}'.format(4) + os.sep + 'Numpy' + os.sep ]
# 	saveFilePath  = 'report' + os.sep + 'plots' + os.sep + 'a{}'.format(questionNumber) + os.sep
# 	listOfPlots = ['bellmanLoss', 'loss',  'reward', 'train_step','test_step']
# 	iterations = [i for i in range(numberOfIterations) if i not in iterationsToAvoid]
# 	fullIterations = [i for i in range(secondNumberOfIterations) if i not in iterationsToAvoid]
...
# 			numpyFilePath = numpyFilePaths[j]
# 			labelPlot = '{} layers'.format(multiplePlotsName[j])
# 			if j != len(numpyFilePaths)-1 or len(numpyFilePaths) == 1:
# 				plot_array = np.load(numpyFilePath + plotName + 'Array_{}'.format(learningRate) + '.npy')[iterations,:]
# 			else:
# 				plot_array = np.load(numpyFilePath + plotName + 'Array_{}'.format(learningRate) + '.npy')[fullIterations,:]
...
# 			maxValue = max(maxValue,np.max(plot_array))
# 			minValue = min(minValue, np.min(plot_array))
# 			if ('test' in plotName) and (j == (len(numpyFilePaths)-1)) and len(numpyFilePaths) >1: 
# 				indexes = np.linspace(0, 1999, num=100).astype(int)
# 			else:
# 				indexes = np.array(range(plot_array.shape[1])).astype(int)		
...
# 	numberOfIterations = 1
# 	secondNumberOfIterations = 100
# 	learningRates = [0.035, 0.00001, 0.015,0.01]
# 	discount = 0.99
# 	multiplePlotsName = ['Q8','Q7', 'Q6', 'Q4']

# 	#### paths and arrays
# 	numpyFilePaths = ['Q{}'.format(questionNumber) + os.sep + 'Numpy' + os.sep, 'Q{}'.format(7) + os.sep + 'Numpy' + os.sep , 'Q{}'.format(6) + os.sep + 'Numpy' + os.sep , 'Q{}'.format(4) + os.sep + 'Numpy' + os.sep ]
# 	saveFilePath  = 'report' + os.sep + 'plots' + os.sep + 'a{}'.format(questionNumber) + os.sep
# 	listOfPlots = ['bellmanLoss', 'loss',  'reward', 'train_step','test_step']
# 	iterations = [i for i in range(numberOfIterations) if i not in iterationsToAvoid]
# 	fullIterations = [i for i in range(secondNumberOfIterations) if i not in iterationsToAvoid]
...
# 			maxValue = max(maxValue,np.max(plot_array))
# 			minValue = min(minValue, np.min(plot_array))
# 			if ('test' in plotName) and (j == (len(numpyFilePaths)-1)): 
# 				indexes = np.linspace(0, 1999, num=100).astype(int)
# 			else:
# 				indexes = np.array(range(plot_array.shape[1])).astype(int)		
...
# def plotQuestion4to8(questionNumber,iterationsToAvoid,numberOfIterations,learningRate,hiddenLayers = None,compareTo=[]):
# 	numpyFilePath = 'Q{}'.format(questionNumber) + os.sep + 'Numpy' + os.sep
# 	saveFilePath  = 'report' + os.sep + 'plots' + os.sep + 'a{}'.format(questionNumber) + os.sep
# 	listOfPlots = ['bellmanLoss', 'loss',  'reward', 'train_step','test_step']
# 	mainListOfPlots = list(listOfPlots)

# 	if hiddenLayers != None:
# 		listOfPlots = ['hiddenLayer_{}_'.format(hiddenLayers) + elem for elem in mainListOfPlots]
# 	discount = 0.99
# 	iterations = [i for i in range(numberOfIterations) if i not in iterationsToAvoid]
...
# 	for i in range(len(listOfPlots)):
# 		plotName = listOfPlots[i]
# 		comparePlotName = mainListOfPlots[i]
# 		print('Q{} : {}-plot'.format(questionNumber,plotName))
# 		fig = plt.figure()
# 		if questionNumber == 4:
# 			plotLabel = ''
# 		else:
# 			plotLabel = 'Q{}'.format(questionNumber)
# 		plot_array = np.load(numpyFilePath + plotName + 'Array_{}'.format(learningRate) + '.npy')[iterations]
# 		max_Value = np.max(plot_array)
# 		plot_mean = plot_array.mean(axis = 0)
# 		plot_std = plot_array.std(axis = 0)
# 		plt.plot(plot_mean, label = plotLabel )
# 		if len(iterations) >1:
# 			plt.fill_between(range(len(plot_mean)),plot_mean+plot_std,plot_mean-plot_std, facecolor = 'green', edgecolor = None,lw=0, alpha = 0.5)
		
# 		for plotToCompareTo, compareLearningRate in compareTo:
# 			tempNumpyFilePath = 'Q{}'.format(plotToCompareTo) + os.sep + 'Numpy' + os.sep
# 			plotLabel = 'Q{}'.format(plotToCompareTo)
# 			temp_plot_array = np.load(tempNumpyFilePath + comparePlotName + 'Array_{}'.format(compareLearningRate) + '.npy')[iterations]
# 			if temp_plot_array.shape[1] > plot_array.shape[1]:
# 				indexes = np.linspace(0, temp_plot_array.shape[1]-1, num=plot_array.shape[1]).astype(int)
# 			else:
# 				indexes = np.array(range(plot_array.shape[1])).astype(int)
				
# 			max_Value = max(max_Value,np.max(plot_array))
# 			plot_mean = temp_plot_array[:,indexes].mean(axis = 0)
# 			plot_std = temp_plot_array[:,indexes].std(axis = 0)
# 			plt.plot(plot_mean, label = plotLabel )
# 			if len(iterations) >1:
# 				plt.fill_between(range(len(plot_mean)),plot_mean+plot_std,plot_mean-plot_std, facecolor = 'green', edgecolor = None,lw=0, alpha = 0.5)
...
# 		if len(iterations)>1:
# 			fig = plt.figure()
# 			plot_mode = np.median(plot_array,axis = 0)
# 			plt.plot(plot_mode, label = str(learningRate))
# 			plt.legend(loc='lower right', fancybox = True, framealpha = 0.5)
# 			plt.xlabel('number of epochs')
# 			plt.ylabel(plotName)
# 			plt.savefig(saveFilePath + plotName + '_median' + '.png')
# 			plt.close(fig)
...
# 	plotName = 'test_reward'
# 	if hiddenLayers != None:
# 		plotName = 'hiddenLayer_{}_'.format(hiddenLayers) + plotName
# 	print('Q{} : {}-plot'.format(questionNumber,plotName))
# 	fig = plt.figure()
# 	if hiddenLayers != None:
# 		arrayString = 'hiddenLayer_{}_test_step'.format(hiddenLayers)
# 	else:
# 		arrayString = 'test_step'
# 	initial_plot_array = np.load(numpyFilePath + arrayString + 'Array_{}'.format(learningRate) + '.npy')[iterations]
...
# plotQuestion4to8(4,[16],100,0.01)
# plotQuestion4to8(5,[],1,0.01,hiddenLayers =30,compareTo=[(4,0.01)])
# plotQuestion4to8(5,[],1,0.01,hiddenLayers =1000,compareTo=[(4,0.01)])
# plotQuestion4to8(6,[],1,0.015,compareTo=[(4,0.01)])
# plotQuestion4to8(7,[],1,0.00001,compareTo=[(4,0.01),(6,0.015)])
# plotQuestion4to8(8,[],1,0.035,compareTo=[(4,0.01),(6,0.015),(7,0.0001)])