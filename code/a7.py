...
plotFolderPath = 'Q7' +  os.path.sep + 'Plot' + os.path.sep
numpyFolderPath = 'Q7' +  os.path.sep +  'Numpy' + os.path.sep
modelFolderPath = 'Q7' +  os.path.sep +  'Model' + os.path.sep
...
def buildNetwork():
	...
	if useBias:
		copyQHiddenBias = tf.Variable(tf.zeros([hiddenSize]))

	copyQWeight = tf.Variable(tf.random_normal([hiddenSize, 2], stddev=std))
	if useBias:
		copyQBias = tf.Variable(tf.zeros([2]))


	hiddenWeightAssign = copyQHiddenWeight.assign(qHiddenWeight)
	if useBias:
		hiddenBiasAssign = copyQHiddenBias.assign(qHiddenBias)
	WeightAssign = copyQWeight.assign(qWeight)
	if useBias:
		BiasAssign = copyQBias.assign(qBias)

	if useBias:
		nextqHiddenMatrix = tf.nn.relu(tf.matmul(nextState, copyQHiddenWeight) + copyQHiddenBias)
	else:
		nextqHiddenMatrix = tf.nn.relu(tf.matmul(nextState, copyQHiddenWeight))

	if useBatchNorm:
		nextqHiddenMatrix = batch_norm_wrapper(nextqHiddenMatrix, is_training_bool)

	if useBias:
		nextQ = tf.matmul(nextqHiddenMatrix, copyQWeight) + copyQBias
	else:
		nextQ = tf.matmul(nextqHiddenMatrix, copyQWeight)
	...
	return currentState, nextState, is_training_bool, reward_value, currentAction, rowIndices, loss, bellman_loss, train_step, qMatrix, hiddenWeightAssign, WeightAssign
...
currentState, nextState, is_training_bool, reward_value, currentAction, rowIndices, loss, bellman_loss, train_step, qMatrix, hiddenWeightAssign, WeightAssign = buildNetwork()
...
for iteration in range(nIterations):
	...
	with tf.Session() as sess :
		...
		for episode in range(nEpisodes):
			...
			updatedString = '     '
			if counter % 5 == 0:
				sess.run([hiddenWeightAssign,WeightAssign])
				updatedString = ' - UP'
