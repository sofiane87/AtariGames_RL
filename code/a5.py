...
################### BUILDING PATH ############################
plotFolderPath = 'Q5' +  os.path.sep + 'Plot' + os.path.sep
numpyFolderPath = 'Q5' +  os.path.sep +  'Numpy' + os.path.sep
modelFolderPath = 'Q5' +  os.path.sep +  'Model' + os.path.sep
...
hiddenSize = 30
if('-hs' in argList):
	hiddenSize = int(argList[argList.index('-hs')+1])
################### Initializing GYM ##########################
gym.envs.register(
...
def savePlots(inputArray, nEpisodes, filename, ylabel, mode = 'step'):
	plot_array = inputArray
	if mode == 'step':
		plt.axis([0, nEpisodes, 0, maximumLength])
	elif mode == 'reward':
		plt.axis([0, nEpisodes, -1, 0])
...
for iteration in range(nIterations):
	count = 0
	bestValue = 0
	saverPath = modelFolderPath + 'hiddenLayer_{}_Model_{}_{}.ckpt'.format(hiddenSize, iteration, learningRate)
	...
	np.save(numpyFolderPath + 'hiddenLayer_{}_bellmanLossArray_{}'.format(hiddenSize, learningRate), bellmanLossArray)
	np.save(numpyFolderPath + 'hiddenLayer_{}_lossArray_{}'.format(hiddenSize, learningRate), lossArray)
	np.save(numpyFolderPath + 'hiddenLayer_{}_rewardArray_{}'.format(hiddenSize, learningRate), rewardArray)
	np.save(numpyFolderPath + 'hiddenLayer_{}_train_stepArray_{}'.format(hiddenSize, learningRate), stepArray)
	np.save(numpyFolderPath + 'hiddenLayer_{}_test_stepArray_{}'.format(hiddenSize, learningRate), testStepArray)


		
savePlots(np.mean(bellmanLossArray, axis = 0), bellmanLossArray.shape[1],plotFolderPath + 'hiddenLayer_{}_bellmanLossPlot_{}'.format(hiddenSize, learningRate), 'bellman loss', mode = '')
savePlots(np.mean(lossArray, axis = 0), lossArray.shape[1], plotFolderPath + 'hiddenLayer_{}_lossPlot_{}'.format(hiddenSize, learningRate), 'loss', mode = '')
savePlots(np.mean(rewardArray, axis = 0),rewardArray.shape[1], plotFolderPath + 'hiddenLayer_{}_rewardPlot_{}'.format(hiddenSize, learningRate), 'cumulative reward', mode = 'reward')
savePlots(np.mean(stepArray, axis = 0),stepArray.shape[1], plotFolderPath + 'hiddenLayer_{}_train_stepPlot_{}'.format(hiddenSize, learningRate), 'train steps before termination', mode = 'step')
savePlots(np.mean(testStepArray, axis = 0),testStepArray.shape[1], plotFolderPath + 'hiddenLayer_{}_test_stepPlot_{}'.format(hiddenSize, learningRate), 'test steps before termination', mode = 'step')
